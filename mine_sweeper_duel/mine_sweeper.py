...
from .get_gold import (ScoreCounter, daily_card_limiter, 
						MAX_GUESS_NUM, EASY_GOLD, NORMAL_GOLD, HARD_GOLD,
						get_user_card_dict, uid2card)
...
async def mine_sweeper(bot, ev):
	...
	with mgr.start(ev.group_id, grid_x, grid_y, mine_num) as ms:
		...
		if ms.end_flag == LOSE :
			...
			gold_msg = ""
			if degree_ != DEGREE_CUSTOM and len(ms.participant) > 0:
				user_card_dict = await get_user_card_dict(bot, ev.group_id)
				gold = 0
				if degree_ == DEGREE_EASY: gold = EASY_GOLD
				if degree_ == DEGREE_NORMAL: gold = NORMAL_GOLD
				if degree_ == DEGREE_HARD: gold = HARD_GOLD
				gold_msg = "以下参与的玩家都获得了" + str(gold) + "金币\n"
				for uid in ms.participant :
					guid = ev.group_id, uid
					if daily_card_limiter.check(guid):
						user_card = uid2card(uid, user_card_dict)
						score_counter = ScoreCounter()
						daily_card_limiter.increase(guid)
						daily_num = daily_card_limiter.get_num(guid)
						gold_msg += (user_card + "(" + str(daily_num) + "/" + str(MAX_GUESS_NUM) + ") ")
